...
async function isBan(user_qq:number, group_id:number) {
...
async function ban(self_authority:number,user_qq:number, group_id:number, ban_time:number) {
...
async function unban(self_authority:number,user_qq:number, group_id:number) {
...
//  考虑顺便增加禁言功能
async function superBan(user_qq:number, group_id:number, ban_time:number) {
  let user_isBan = await isBan(user_qq,group_id)
  if(user_isBan){
...
async function superUnban(user_qq:number, group_id:number) {
  let user_isBan = await isBan(user_qq,group_id)
  if(!user_isBan){
    return "用户未被ban"
  } else {
...
//  若未被禁言，则返回false
//  若被禁言，则返回禁言时间
async function banTime(user_qq:number, group_id:number) {
  let user = await prisma.ban_users.findUnique({
    where: {
      user_qq_group_id: {
        user_qq: user_qq,
...
//返回当前群里被ban的人数
//  接受当前群号
async function banNum(group_id:number) {
...
class banOprate {
  ...
  constructor() {
    ...
    this.superUnban = superUnban
    this.banTime = banTime
    this.banNum = banNum
  }
}

export default banOprate