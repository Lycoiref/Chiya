...
class User {
    ...
    getSignImage = async (random_num: string, checkin_time_last_str: string, impression: string, user: any, userInfo:any) => {
        ...
        let lyf = `http://127.0.0.1:5140/sign?random_num=${random_num}&checkin_time_last_str=${checkin_time_last_str}&impression=${impression}&${query}&user_avatar=${userInfo.avatar}&user_name=${userInfo.username}`
        console.log(lyf)
        await page.goto(lyf)
        let str
        // 等待图片加载
